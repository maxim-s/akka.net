...
namespace Akka.DistributedData.Proto
{
    ...
    public class ReplicatorMessageSerializer : SerializerWithStringManifest, ISerializationSupport
    {
        ...
        public override string Manifest(object obj)
        {
            ...
            if (obj is Write) { return WriteManifest; }
            if (obj is WriteAck) { return WriteAckManifest; }
            if (obj is Read) { return ReadManifest; }
            if (obj is ReadResult) { return ReadResultManifest; }
            if (obj is Internal.Status) { return StatusManifest; }
            if (obj is Gossip) { return GossipManifest; }
            if (obj is IGet) { return GetManifest; }
            if (obj is IGetSuccess) { return GetSuccessManifest; }
            if (obj is IChanged) { return ChangedManifest; }
            if (obj is INotFound) { return NotFoundManifest; }
            if (obj is IGetFailure) { return GetFailureManifest; }
            if (obj is ISubscribe) { return SubscribeManifest; }
            if (obj is IUnsubscribe) { return UnsubscribeManifest; }
            throw new ArgumentException("Unable to serialize {0}", obj.GetType().Name);
        }
        ...
        public override byte[] ToBinary(object obj)
        {
            ...
            if (obj is Write) { return WriteToProto((Write)obj).ToByteArray(); }
            if (obj is WriteAck) { return dm.Empty.DefaultInstance.ToByteArray(); }
            if (obj is Read) { return ReadToProto((Read)obj).ToByteArray(); }
            if (obj is ReadResult) { return ReadResultToProto((ReadResult)obj).ToByteArray(); }
            if (obj is Internal.Status) { return StatusToProto((Internal.Status)obj).ToByteArray(); }
            if (obj is Gossip) { return GossipToProto((Gossip)obj).ToByteArray(); }
            if (obj is IGet) { return GetToProto((IGet)obj).ToByteArray(); }
            if (obj is IGetSuccess) { return GetSuccessToProto((IGetSuccess)obj).ToByteArray(); }
            if (obj is IChanged) { return ChangedToproto((IChanged)obj).ToByteArray(); }
            if (obj is INotFound) { return NotFoundToProto((INotFound)obj).ToByteArray(); }
            if (obj is IGetFailure) { return GetFailureToProto((IGetFailure)obj).ToByteArray(); }
            if (obj is ISubscribe) { return SubscribeToProto((ISubscribe)obj).ToByteArray(); }
            if (obj is IUnsubscribe) { return UnsubscribeToProto((IUnsubscribe)obj).ToByteArray(); }
            throw new ArgumentException("Unable to serialize {0}", obj.GetType().Name);
        }
        ...
        private Internal.Status StatusFromBinary(byte[] bytes)
        {
        ...
        private Gossip GossipFromBinary(byte[] bytes)
        {
            ...
            var entries = gossip.EntriesList.Select(x =>
                {
                    var key = x.Key;
                    var env = DataEnvelopeFromProto(x.Envelope);
        ...
        private object GetSuccessFromBinary(byte[] bytes)
        {
            ...
            return Activator.CreateInstance(invokeType, key, request, data);
        }
        ...
        private object NotFoundFromBinary(byte[] bytes)
        {
            ...
            return Activator.CreateInstance(invokeType, key, request);
        }
        ...
        private object GetFailureFromBinary(byte[] bytes)
        {
            ...
            return Activator.CreateInstance(invokeType, key, req);
        }
        ...
        private object SubscribeFromBinary(byte[] bytes)
        {
            ...
            return Activator.CreateInstance(invokeType, key, actorRef);
        }
        ...
        private object ChangedFromBinary(byte[] bytes)
        {
            ...
            return Activator.CreateInstance(invokeType, key, data);
        }
