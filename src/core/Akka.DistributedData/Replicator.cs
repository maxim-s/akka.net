...
namespace Akka.DistributedData
{
    public class Replicator : ActorBase
    {
        ...
        readonly MethodInfo _deleteMethodInfo;


        readonly MethodInfo _getMethodInfo;
        ...
        readonly MethodInfo _subscribeMethodInfo;

        readonly Dictionary<string, ISet<IActorRef>> _subscribers = new Dictionary<string, ISet<IActorRef>>();
        readonly MethodInfo _unsubscribeMethodInfo;
        readonly MethodInfo _updateMethodInfo;
        long _allReachableClockTime;
                          ...
                          .With<IGet>(g => 
                              {
                                  ...
                                  genericMethod.Invoke(this, new[] { g.Key, g.Consistency, g.Request });
                              })
                          .With<IUpdate>(u => 
                              {
                                  ...
                                  genericMethod.Invoke(this, new[] { u.Key, u.Modify, u.Consistency, u.Request });
                              })
        ...
        private void ReceiveGet<T>(Key<T> key, IReadConsistency consistency, object req) where T : IReplicatedData
        {
            ...
            {
                Context.ActorOf(ReadAggregator<T>.GetProps(key, consistency, req, _nodes, localValue, Sender).WithDispatcher(Context.Props.Dispatcher));
            }
        }
        ...
        private void ReceiveDelete<T>(Key<T> key, IWriteConsistency consistency) where T : IReplicatedData
        {
            ...
            if (contained)
            {
                ...
                else
                {
                    ...
                    {
                        Context.ActorOf(WriteAggregator<T>.GetProps(key, DeletedEnvelope, consistency, null, _nodes, Sender).WithDispatcher(Context.Props.Dispatcher));
                    }
                }
            }
        }
